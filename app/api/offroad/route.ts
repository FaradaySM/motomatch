import { recommendOffroad, type OffroadRequest } from "@/lib/offroad/recommender";
...
function parseBody(body: unknown): { ok: true; data: OffroadRequest } | { ok: false; error: string } {
  ...
  if (!isFiniteNumber(dias_viaje) || dias_viaje <= 0 || dias_viaje > 60) {
    return { ok: false, error: "dias_viaje debe ser un número > 0 (máx 60)." };
  }
  if (!isFiniteNumber(altura_piloto_cm) || altura_piloto_cm < 140 || altura_piloto_cm > 210) {
    return { ok: false, error: "altura_piloto_cm debe estar entre 140 y 210." };
  }
  if (!isFiniteNumber(peso_piloto_kg) || peso_piloto_kg < 40 || peso_piloto_kg > 180) {
    return { ok: false, error: "peso_piloto_kg debe estar entre 40 y 180." };
  }
